...
export async function GET(request: NextRequest) {
  try {
    ...
    return new Response(JSON.stringify({ entity: 'events', count: data.length, data }, null, 2), {
      headers: {
        'Content-Type': 'application/json; charset=utf-8',
        'Content-Disposition': `attachment; filename="wwtrail-events-${new Date().toISOString().split('T')[0]}.json"`,
      },
    });
  } catch (error) {
