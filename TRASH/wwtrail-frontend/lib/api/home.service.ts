...
class HomeService {
  ...
  async getConfigurationById(id: string): Promise<HomeConfiguration> {
    const response = await apiClientV2.get<{ data: HomeConfiguration }>(`/home/config/${id}`);
    return response.data.data;
  }
  ...
  async createConfiguration(data: UpdateHomeConfigurationDTO): Promise<HomeConfiguration> {
    const response = await apiClientV2.post<{ data: HomeConfiguration }>('/home/config', data);
    return response.data.data;
  }
  ...
  async updateConfiguration(
    ...
    const response = await apiClientV2.patch<{ data: HomeConfiguration }>(
      `/home/config/${id}`,
      data
    );
    return response.data.data;
  ...
  async updateFullConfiguration(
    ...
    const response = await apiClientV2.put<{ data: HomeConfiguration }>(
      `/home/config/${id}/full`,
      data
    );
    return response.data.data;
  ...
  async deleteConfiguration(id: string): Promise<void> {
    await apiClientV2.delete(`/home/config/${id}`);
  }
  ...
  async reorderBlocks(
    ...
    const response = await apiClientV2.post<{ data: HomeConfiguration }>(
  ...
  async createBlock(configId: string, data: CreateHomeBlockDTO): Promise<HomeBlock> {
    const response = await apiClientV2.post<{ data: HomeBlock }>(
  ...
  async updateBlock(blockId: string, data: UpdateHomeBlockDTO): Promise<HomeBlock> {
    const response = await apiClientV2.patch<{ data: HomeBlock }>(`/home/blocks/${blockId}`, data);
    return response.data.data;
  }
  ...
  async deleteBlock(blockId: string): Promise<void> {
    await apiClientV2.delete(`/home/blocks/${blockId}`);
  }
